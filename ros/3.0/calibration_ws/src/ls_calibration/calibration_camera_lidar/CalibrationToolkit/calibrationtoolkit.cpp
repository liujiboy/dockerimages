...
bool CalibrateCameraChessboardROS::grabCalibData()
{
    ...
    qDebug()<<"channels"<<calibimage.channels();
    if(calibimage.channels()!=1)
    {
        cv::Mat image_gray;
        cv::cvtColor(calibimage, image_gray, cv::COLOR_BGR2GRAY);
        cv::cornerSubPix(image_gray,grid2dpoint,cv::Size(11, 11), cv::Size(-1, -1),cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
    }
    else{
